...
export default function App() {
  ...
  const [values, setValues] = useState({id: 0, fname: "", lname: "", bio: ""})
  // ✨ Create a third state to track the values of the inputs

  useEffect(() => {
    if (editing) {
      const filter = members.filter(mem => mem.id === editing)
      const member = filter[0]
      setValues({id: member.id, fname: member.fname, lname: member.lname, bio: member.bio})
    }
    // setValues({...values, fname: member.fName, lname: member.lname, bio: member.bio})
  ...
  const submitNewMember = () => {
    ...
    setMembers(members.concat({id: getId(), fname: fname, lname: lname, bio: bio}))
  }
